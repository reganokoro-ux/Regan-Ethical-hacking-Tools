...
def configure():
    ...
    if config["injection_location"] == "post":
        ...
        dyn = input("Try to auto-extract hidden/CSRF fields? (y/n, default n): ").strip().lower()
        if dyn == 'y':
    ...
    manual = input("Do you want to set prefix/suffix manually? (y/n, default n): ").strip().lower()
    if manual == 'y':
    ...
    if oracle_choice == 'y':
        ...
        config["indicator_type"] = input("Indicator type (contains/length_gt/length_lt/status_code/header, default contains): ").strip() or "contains"
        if config["indicator_type"] in ["length_gt", "length_lt", "status_code"]:
            config["indicator_value"] = input("Value for indicator: ").strip()
        elif config["indicator_type"] == "header":
            config["indicator_value"] = input("Header name that differs: ").strip()
    ...
    enc = input("Encoding (none/url/double_url/unicode, default none): ").strip().lower()
    config["encoding"] = enc if enc in ["url", "double_url", "unicode"] else "none"

    obf = input("Obfuscation mode (none/comments/case/operators/parentheses/null_byte/random, default none): ").strip().lower()
    config["obfuscation_mode"] = obf if obf in ["comments", "case", "operators", "parentheses", "null_byte", "random"] else "none"
    ...
    conc = input("\nConcurrency (number of parallel requests for detection, default 5): ").strip()
    if conc.isdigit():
    ...
    # ** NEW: Ask whether to run auto-detection in Phase 1 **
    auto_detect_choice = input("Run auto-detection in Phase 1? (y/n, default y): ").strip().lower()
